...
import eu.qualify.food4me.decisiontree.*
...
class GenerateAdviceServiceIntegrationSpec extends IntegrationSpec {
    ...
    void "test advice generation with duplicate advices"() {
        given: "a request to advise on omega3 intake"
        advisables = [ Property.findByEntityAndPropertyGroup("Omega-3", Property.PROPERTY_GROUP_NUTRIENT) ]
        ...
        and: "an extra advice that will be generated for the same parameters"
            new Advice(subject: omega3Intake, code: "L3.4.004", text: "Another text")
                .addToConditions( new AdviceCondition( subject: geneFADS1, status: Status.STATUS_RISK ) )
                .addToConditions( new AdviceCondition( subject: omega3Biomarker, status: Status.STATUS_LOW ) )
                .addToConditions( new AdviceCondition( subject: omega3Intake, status: Status.STATUS_OK ) )

        when: "the advice is generated"
            def advices = generateAdviceService.generateAdvice(measurements, measurementStatus, advisables)

        then: "the list of advices contains only a single advice"
            assert advices?.size() == 1
            assert advices[0].code == "L3.4.004"
    }
